...
{
	// Reset + re-register THIS instance's options (with its own callbacks)
	// before parsing. Without the reset, every openEMS() constructed in a
	// single Python process appends another copy of the option set to the
	// global registry, so the 2nd engine='cuda' Run() throws
	// "--engine=cuda is ambiguous". Re-registering here makes the GPU
	// workflow behave exactly like the CPU one across parameter sweeps,
	// optimizers, and repeated notebook runs in one interpreter.
...
//		ProcField->AddStep(FDTD_Op->Exc->Volt_delay[n]+maxExcite);
	...
	// Throttle the end-criterion energy check to a bounded, timestep-based
	// cadence (deterministic and engine-speed-independent) -- frequent enough
	// to early-stop near convergence, sparse enough that the whole-grid energy
	// reduction is not a per-chunk tax on large or fixed-length runs.
	unsigned int lastEnergyCheck = 0;
	unsigned int energyCheckInterval = std::max<unsigned int>(200, (unsigned int)(NrTS/500));
	...
	while ((FDTD_Eng->GetNumberOfTimesteps()<NrTS) && (change>endCrit) && !CheckAbortCond())
	{
		...
		// below. Without this a run that finishes in under 4s -- typically the
		// (fast) GPU engine -- never updates 'change' and always burns the full
		// NrTS instead of early-stopping at convergence like a slower CPU run.
		// Basing the check on timesteps (not wall-clock) also makes the stop
		// point deterministic and identical across the CPU and CUDA engines.
		if (Eng_Ext_SSD==NULL)
		{
			unsigned int cts = FDTD_Eng->GetNumberOfTimesteps();
			if (cts - lastEnergyCheck >= energyCheckInterval)
			{
				lastEnergyCheck = cts;
