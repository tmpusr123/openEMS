...
bool Operator_Ext_WaveguideAbsorber::BuildExtension()
{
	...
	// Layer 0 is closest to domain boundary (farthest from source)
	// Layer 1 (if 2-layer) is between source and layer 0
	if (m_absLayers == 1)
	{
		// Absorber at src +/- 1 cell
		m_E_abs_line[0] = m_srcLine + m_dir;
		m_H_abs_line[0] = m_E_abs_line[0] - m_dir;
	}
	else if (m_absLayers == 2)
	{
		// Outer absorber at src +/- 2 cells
		m_E_abs_line[0] = m_srcLine + 2 * m_dir;
		m_H_abs_line[0] = m_E_abs_line[0] - m_dir;
		// Inner absorber at src +/- 1 cell
	...
	// 5. Compute numerical wave impedance: Zw = eta0 * sqrt(1 - S^2/4)
	...
	double S = __C0__ * dT / dL;
	m_Zw = sqrt(__MUE0__ / __EPS0__) * sqrt(1.0 - S * S / 4.0);
